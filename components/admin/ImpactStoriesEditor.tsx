...
export default function ImpactStoriesEditor() {
  ...
  function commit() {
    if (editing === -1) setStories(m => [...m, { ...draft }])
    else setStories(m => m.map((x, i) => i === editing ? { ...draft } : x))
    setEditing(null); setDraft(BLANK)
  }
  function remove(i: number) { setStories(m => m.filter((_, j) => j !== i)) }
  ...
  const fields: [keyof Story, string][] = [
    ['name', 'Full Name'],
    ['role', 'Role / Title'],
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: '.78rem', color: '#8a9a8f' }}>Stories shown on the public /impact-content page.</p>
        </div>
            ...
            {editing === i ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {fields.map(([k, label]) => (
                    <div key={k} style={k === 'desc' ? { gridColumn: '1 / -1' } : undefined}>
                      <label style={lbl}>{label}</label>
                      {k === 'desc'
                        ? <textarea style={{ ...inp, minHeight: 72, resize: 'vertical' }} value={draft[k]} onChange={e => setDraft(d => ({ ...d, [k]: e.target.value }))} />
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  ...
                  <div style={{ fontSize: '.8rem', color: '#4a5a4f', marginTop: 4, maxWidth: 480 }}>{story.desc}</div>
                </div>
      ...
      {stories.length === 0 && editing !== -1 && (
        <p style={{ color: '#8a9a8f', fontSize: '.85rem', marginTop: 12 }}>
          No stories yet. Add stories to display them on the public /impact-content page.
        </p>
      )}
    </div>
  )
